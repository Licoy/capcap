#!/usr/bin/env bash
# Bump CFBundleShortVersionString / CFBundleVersion in app + share-extension
# Info.plist files, commit, and create an annotated vX.Y.Z tag that triggers
# .github/workflows/release.yml.
#
# Modeled on https://github.com/Licoy/codex-runway/blob/main/bump.sh
#
# Usage:
#   ./bump.sh -v 1.0.1       # commit + tag locally
#   ./bump.sh -v 1.0.1 -p    # also push branch and tag to origin
set -euo pipefail

usage() {
  printf 'Usage: %s -v 1.0.1 [-p]\n' "$(basename "$0")" >&2
  exit 2
}
...
APP_PLIST="capcap/App/Info.plist"
EXT_PLIST="capcap-share-extension/Info.plist"
PLISTS=("$APP_PLIST" "$EXT_PLIST")
TAG="v$VERSION"
...
# If plists are already at the requested version (e.g. fork baseline 1.0.0
# landed in a prior commit), only create the release tag — no version commit.
if [[ "$CURRENT_VERSION" == "$VERSION" ]]; then
  CURRENT_BUILD="$(/usr/libexec/PlistBuddy -c 'Print :CFBundleVersion' "$APP_PLIST")"
  printf 'Info.plist already at %s (build %s); creating tag only.\n' "$VERSION" "$CURRENT_BUILD"
  ...
  printf 'Tagged %s (version already %s).\n' "$TAG" "$VERSION"
  if [[ "$PUSH" -eq 0 ]]; then
...
for PLIST in "${PLISTS[@]}"; do
  VERSION="$VERSION" BUILD="$BUILD" perl -0pi -e '
    ...
    die "Missing version keys in '"$PLIST"'\n" unless $short && $build;
  ' "$PLIST"
